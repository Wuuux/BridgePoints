...
function historia(nr){
  var t = "";
  var i = 1;
  var historia_ID = document.getElementById('historia');
  while (i <=nr){
  t += "<div>"+pobierzIoblicz(i)+"</div>";
  i++;
  }
  historia_ID.innerHTML = t;
}
...
function tabela(gracz,pWE,pNS,kWy,kKo,ko,pR,l,n,w){
  ...
  if (gracz=='WE') {tekst += "<tr><td>WE:</td><td> " + (suma)+ "</td><td></td></tr>";}
  if (gracz=='NS') {tekst += "<tr><td>NS:</td><td></td><td>"+ (suma)+"</td></tr>";}
  tekst += "</table>";
  ...
  impWE_ID.innerHTML = "WE: " + (sumaImpWE);
  impNS_ID.innerHTML = "NS: " + (sumaImpNS);
  return tekst;
};
